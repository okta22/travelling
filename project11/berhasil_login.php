...
<section class="input-data" id="input">
...
<?php
	if(isset($_POST['submit'])){
		...
		$nomer = addslashes($_POST['phone']);
		$email = addslashes($_POST['email']);
        $alamat = addslashes($_POST['adress']);
        $travel = addslashes($_POST['travel']);
        $date = addslashes($_POST['date']);
        $time = addslashes($_POST['time']);

        $cek = mysqli_query($conn, "SELECT Nama FROM tb_orang WHERE Nama='". $nama ."' ");

		if(mysqli_num_rows($cek)>0){
			echo "<script>alert('Nama sudah digunakan,silahkan isi ulang data anda!!!')</script>";
				}else{

					    $simpan	= mysqli_query($conn, "INSERT INTO tb_orang (Nama, No_Hp, Email, Alamat, Travel, Tanggal, Waktu ) VALUES ('".$nama."', '".$nomer."', '".$email."', '".$alamat."', '".$travel."', '".$date."', '".$time."')");
